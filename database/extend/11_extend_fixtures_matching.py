"""
database/11_extend_fixtures_matching.py

EXTEND-ONLY version of `public.fixtures_matching`.

Creates/extends `public.fixtures_matching` which maps:
...
Matching logic (same as full script):
- Uses CSV mappings:
...
EXTEND behavior:
- Processes ONLY fixtures that are not yet in public.fixtures_matching
  OR that have oa_event_id IS NULL (unmatched rows).
- Does NOT touch already-matched rows (oa_event_id not null).

Usage:
  python -m database.11_extend_fixtures_matching
  python -m database.11_extend_fixtures_matching --limit 1000 --window-hours 12
"""
...
def upsert_fixtures_matching(engine, rows: List[Dict[str, Any]]) -> int:
    """
    Upsert rows (same logic as full script).
    Extend-script only passes rows for fixtures that are missing/unmatched.
    """
    if not rows:
        return 0
...
# ----------------------------
# DB reading (extend-only candidates)
# ----------------------------
def fetch_extend_candidate_fixtures(engine, league_ids: List[int], limit: int) -> List[FixtureRow]:
    """
    EXTEND candidate fixtures:
    - fixtures whose fixture_id is not in fixtures_matching
      OR fixtures_matching.oa_event_id IS NULL (unmatched)
    """
    sql = text(
        """
        SELECT
        ...
        WHERE f.league_id = ANY(:league_ids)
          ...
          AND (fm.fixture_id IS NULL OR fm.oa_event_id IS NULL)
...
def main() -> None:
    ...
    ap.add_argument("--limit", type=int, default=500, help="Only process this many fixtures (extend batch size).")
    ...
    fixtures = fetch_extend_candidate_fixtures(engine, league_ids=league_ids, limit=int(args.limit))

    print(f"[FIXTURE MATCH EXTEND] candidate fixtures (missing/unmatched): {len(fixtures)} (limit={args.limit})")
    if not fixtures:
        print("[FIXTURE MATCH EXTEND] Nothing to do.")
        return
    ...
    print("\nDone.")
    print(
        f"[FIXTURE MATCH EXTEND] processed={len(fixtures)} matched={matched} "
        f"to_upsert={len(to_upsert)} upserted={up}"
    )
    print(
        f"[FIXTURE MATCH EXTEND] skipped_missing_mapping={skipped_missing_mapping} "
        f"no_event_found={no_event_found} api_failed={api_failed}"
    )
