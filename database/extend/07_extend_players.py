"""
database/players/07_extend_players.py

EXTEND-ONLY loader for `public.players`.

Creates + maintains the `public.players` table with:
- player_id (PK)
...
Behavior:
- Only fetches players that are present in public.lineups but missing in public.players
- UPSERTs player_name (updates only if changed; bumps updated_at only on change)
- Processes in batches to reduce DB writes
"""
...
def main() -> None:
    ...
    print(f"[PLAYERS-EXTEND] unique player_ids in lineups: {len(lineup_ids)}")
    print(f"[PLAYERS-EXTEND] already in public.players: {len(existing_ids)}")
    print(f"[PLAYERS-EXTEND] missing to fetch: {len(missing)}")

    if not missing:
        print("[PLAYERS-EXTEND] Nothing to extend. public.players is up to date.")
    ...
    for i, player_id in enumerate(missing, start=1):
        try:
            ...
            if not name:
                failed += 1
                print(f"[PLAYERS-EXTEND] Warning: player_id={player_id} returned empty name, skipping")
                continue
            ...
            if i % 200 == 0 or i == len(missing):
                print(
                    f"[PLAYERS-EXTEND] progress {i}/{len(missing)} "
                    f"fetched={fetched} failed={failed} upserted={upserted_total}"
                )
            ...
            failed += 1
            print(f"[PLAYERS-EXTEND] Warning: failed player_id={player_id}: {e}")
            time.sleep(0.3)
            continue
    ...
    print("\nDone.")
    print(f"[PLAYERS-EXTEND] fetched={fetched} failed={failed} upserted_rows={upserted_total}")
    print("Table: public.players")
